import { b as watch$1, v as debounce, x as createProxyServer } from "../_libs/c12+rc9+rou3+nypm+klona+citty.mjs";
import { B as T, V as a, ct as join$1, dt as resolve$1, g as writeDevBuildInfo, j as src_default, ot as extname$1 } from "../_build/common.mjs";
...
async function fetchAddress(addr, input, inputInit) {
	...
	const res = await new Promise((resolve, reject) => {
		...
		req.on("error", reject);
		if (init.body instanceof ReadableStream) Readable.fromWeb(init.body).pipe(req);
		else if (init.body) req.end(init.body);
		else req.end();
	});
	const headers = new Headers();
	for (const [key, value] of Object.entries(res.headers)) {
		if (key === "transfer-encoding" || key === "keep-alive") continue;
