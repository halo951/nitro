import "./_common.mjs";
import { M as formatCompatibilityDate, b as watch$1, p as assetsPlugin, v as debounce } from "./_libs/c12+rc9+rou3+nypm+klona+citty.mjs";
import { B as T, D as prepare, I as prettyPath, O as copyPublicAssets, V as a, at as dirname$1, ct as join$1, d as libChunkName, dt as resolve$1, f as baseBuildConfig, h as writeBuildInfo, it as basename$1, l as NODE_MODULES_RE, m as getBuildInfo, n as baseBuildPlugins, nt as resolveModulePath, st as isAbsolute$1, u as getChunkName, ut as relative$1 } from "./_build/common.mjs";
...
const getBundlerConfig = async (ctx) => {
	...
	if (ctx._isRolldown) {
		...
		const inject = (await import("./_libs/c12+rc9+rou3+nypm+klona+citty.mjs").then((n) => n.c)).default;
		const alias = (await import("./_libs/c12+rc9+rou3+nypm+klona+citty.mjs").then((n) => n.d)).default;
	...
	return {
		...
		async configurePreviewServer(server) {
			...
			if (buildInfo.preset.includes("cloudflare")) {
				if (!buildInfo.commands?.preview) throw this.error(`No nitro build preview command found for the "${buildInfo.preset}" preset.`);
			...
			if (buildInfo.publicDir) {
				...
				const staticHandler = serveStatic({ dir: join$1(outputDir, buildInfo.publicDir) });
				server.middlewares.use(async (req, res, next) => {
					const staticRes = await staticHandler(new NodeRequest({
						req,
						res
					}), () => void 0);
					if (staticRes) await sendNodeResponse(res, staticRes).catch(next);
					else next();
				});
			}
			if (buildInfo.serverEntry) {
				const { loadServerEntry } = await import("srvx/loader");
				const entryPath = resolve$1(outputDir, buildInfo.serverEntry);
				const entry = await loadServerEntry({ entry: entryPath });
				if (entry.notFound || !entry.fetch) throw new Error(`Cannot load nitro server entry: ${entryPath}`);
				server.middlewares.use(async (req, res, next) => {
...
async function loadPreviewDotEnv(root) {
	const { loadDotenv } = await import("./_libs/c12+rc9+rou3+nypm+klona+citty.mjs").then((n) => n.F);
...
async function runPreviewCommand(opts) {
	...
	const { createProxyServer } = await import("./_libs/c12+rc9+rou3+nypm+klona+citty.mjs").then((n) => n.S);
